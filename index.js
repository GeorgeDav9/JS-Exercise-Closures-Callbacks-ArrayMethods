...
*/
  function processSum(numberList, callback) {
    return callback(numberList.reduce((num1, num2) => num1 + num2));
    }   
/**
...
function firstNamesAllCaps(runners) {
 const upCaseNames = runners.map(function(currentValue) {
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  const shirtSizer = runners.filter(function(currentValue){
...
function tallyUpDonations(runners) {
  const tallyCounter = runners.reduce(function (accumulator, cuurentValue) {
 ...
 * etc
*/
function counterMakerWithLimit(element) {
  let count = -1;
  return function() {
    if (count < element) {
      return count += 1;
